...
initializeMaps();function getOffset(el){const rect=el.getBoundingClientRect();return{left:rect.left+window.scrollX,top:rect.top+window.scrollY};}
u(document).on("click",".bible-ref",function(e){if(!self.fetch){return;}
e.preventDefault();u(".verse-popper").remove();var link=u(this).first();var ref=u(this).attr("href").substring(1);fetch("/api/v1.0/bibletext/"+ref).then(function(response){if(response.status!==200){window.location="/"+ref;}
response.json().then(function(data){u("#resource_list").first().insertAdjacentHTML("afterend",'<div class="verse-popper" style="left: '+getOffset(link).left+'px; top: '+getOffset(link).top+'px;"><header>'+data.title+'<a class="close"></a></header><div class="popper-body">'+data.text+'<a class="study-verse" href="/'+ref+'">Study verse »</a></div></div>');});}).catch(function(error){window.location="/"+ref;});});u(document).on("click",".verse-popper .close",function(){u(".verse-popper").remove();});u(document).on("click",".overlay.menu",function(){closeMenu();});u(document).on("click",".resource.expand .panel-heading",function(e){u(this).closest(".resource").removeClass("expand");});u(document).on("click",".resource:not(.expand)",function(e){u(this).addClass("expand");});function loadVerse(ref,raw=false){u(".verse .panel-body").html('<span class="loading-animation"><b>•</b><b>•</b><b>•</b></span>');u("#search").first().blur();closeMenu();url="/api/v1.0/verse/"+ref;if(!raw){window.history.pushState(ref,null,ref);}
