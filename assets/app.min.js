...
u(document).on("click",".verse-popper .close",function(){u(".verse-popper").remove();});u(document).on("click",".overlay.menu",function(){closeMenu();});u(document).on("click",".resource.expand .panel-heading",function(e){u(this).closest(".resource").removeClass("expand");});u(document).on("click",".resource:not(.expand)",function(e){u(this).addClass("expand");});function loadVerse(ref,raw=false,updateState=true){u(".verse .panel-body").html('<span class="loading-animation"><b>•</b><b>•</b><b>•</b></span>');u("#search").first().blur();closeMenu();var limit=8;url="/api/v1.0/verse/"+ref+"/"+limit;if(!raw&&updateState!=false){window.history.pushState(ref,null,ref);}
...
initializeMaps();u(".history-list").prepend('<li><a href="/'+ref+'" class="dropdown-item ref-link">'+data.text_ref+'</a></li>');u(".history-list").each(function(index){if(u(this).find("li").length>10){u(this).find("li").last().remove();}});}else{}};request.onerror=function(){};request.send();}
