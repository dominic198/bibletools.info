var text_ref=u(".text-ref");if(text_ref.length>0){u("#search").first().value=text_ref.text();u("#clear").removeClass("hidden");}
u(".open-menu").on("click",function(){openMenu();});function initializeMaps(){document.querySelectorAll(".map .panel-body a").forEach(function(elem){elem.onclick=function(e){e.preventDefault();var src=elem.getAttribute("href");var html='<img src="'+src+'">';basicLightbox.create(html).show();};});}
initializeMaps();u(document).on("click",".overlay.menu",function(){closeMenu();});u(document).on("click",".resource.expand .panel-heading",function(e){u(this).closest(".resource").removeClass("expand");});u(document).on("click",".resource:not(.expand)",function(e){u(this).addClass("expand");});function loadVerse(ref,raw=false){u(".verse .panel-body").html('<span class="loading-animation"><b>•</b><b>•</b><b>•</b></span>');u("#search").first().blur();closeMenu();if(raw){url="/resources/json/query/"+ref;}else{window.history.pushState(ref,null,ref);url="/resources/json/"+ref;}
...
u("#search").first().value=data.text_ref;u(".verse .panel-body").html(data.verse);u(".next-verse").attr("href","/"+data.nav.next);u(".prev-verse").attr("href","/"+data.nav.prev);u(".prev-verse").toggleClass("hidden",data.nav.prev==false);u(".next-verse").toggleClass("hidden",data.nav.next==false);u("h2 .text-ref").text(data.text_ref);u("#resource_list .resource").remove();data.main_resources.forEach(function(resource,index){u("#resource_list .left-column").append('<div class="panel panel-modern resource" data-index-id="'+resource.id+'"><div class="panel-heading"><div class="author-icon '+resource.logo+'"></div><div class="resource-info"><strong>'+resource.author+'</strong><br><small>'+resource.source+'</small></div></div><div class="panel-body">'+resource.content+'</div><div class="panel-footer"><small>Was this helpful?</small><a class="mark-unhelpful"></a><a class="mark-helpful"></a></div></div>');});data.sidebar_resources.forEach(function(resource,index){u("#resource_list .right-column").append('<div class="panel panel-modern resource '+resource.class+'"><div class="panel-heading"><strong>'+resource.source+'</strong></div><div class="panel-body">'+resource.content+'</div></div>');});initializeMaps();u(".history-list").prepend('<li><a href="/'+ref+'" class="dropdown-item ref-link">'+data.text_ref+'</a></li>');u(".history-list").each(function(index){if(u(this).find("li").length>10){u(this).find("li").last().remove();}});}else{}};request.onerror=function(){};request.send();}
